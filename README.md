...
**Watch an AI rethink the rules of a hidden world—and inspect the evidence behind the change.**

JUMP is a live-first research demo about theory revision. Six objects move in a small 2D world. The AI sees their motion but not the rule underneath it. The demo is designed to show the AI's original prediction, the evidence that breaks it, the alternative world it can inspect, and the explanation it gives next.

> **Status — 13 August 2026:** JUMP is unfinished. It is being developed as a research instrument and AO Agents Hackathon submission. This repository reports no scientific result, no passed behavioral or mechanistic model gate, and no public model release. The live Space and inference endpoint are not deployed.
...
Each JUMP world contains six objects with unobserved binary types. Whether a pair attracts or repels depends on whether those types match; the distance exponent comes from a finite allowlist. Because the simulator knows the ground truth, every answer can be scored exactly.

## The six-object thought experiment

A visitor chooses a supported thought experiment and starts a real Modal-backed run. The interface is intended to show the model's committed prediction, contradictory motion, revised answer, decoded scene, run identity, revisions, timing, and artifact hashes. If the live request fails, the product reports the failure before offering a clearly labeled recorded artifact; it never silently presents a recording as a new run.

The three product flows are a matched World A/World B latent swap, a falsified-prior sequence, and hidden-law discovery. JUMP refuses unrelated requests. It is a bounded synthetic-physics instrument, not a general scientific assistant.
...
- No public adapter, live inference endpoint, or Hugging Face Space is verified. Live-first integration remains active.
...
For a matched observed world, JUMP compares ordinary model inputs with a learned world-state latent supplied through a gated residual-stream adapter while the base model remains frozen.

```mermaid
flowchart LR
    O[Observed frames] --> M[Frozen multimodal model]
    O --> W[Executable world model]
    W --> Z[World-state latent Z]
    Z --> A[Gated adapter]
    A --> M
    M --> S[Structured answer]
...
- A scoreable frozen-Gemma behavioral baseline, a validated latent adapter, a passed two-seed swap gate, a deployed Space, a verified live endpoint, a confirmatory manifest, and any Track R result remain unreported. Recorded development fixtures are not model results.
...
- **Private adapter artifact:** authenticated inspection verified the adapter-only repository [`Perfect7613/jump-gemma-adapter`](https://huggingface.co/Perfect7613/jump-gemma-adapter) at immutable revision [`32a7006f`](https://huggingface.co/Perfect7613/jump-gemma-adapter/tree/32a7006f9f8a8fbdb792bb6ee08956b255efee55). It contains the pilot and smoke adapters, manifests, dataset records, tokenization audit, results, and checksums; it excludes base weights. The repository is private, is not a release, and is not demo-ready. No inference endpoint exists.
...
This integration is not present on `origin/main` and has not been deployment-tested for this README. Until its implementation PR and Space/Modal deployment are inspected, “live-first” describes the required submission behavior, not an available endpoint.
...
- **Release:** no tagged scientific release, public dataset/model repository, verified Space URL, live Modal integration, or result-bearing paper is declared here. Development fixtures are not releases or results.
