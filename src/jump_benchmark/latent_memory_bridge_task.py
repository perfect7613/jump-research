...
from .latent_memory_bridge import SOURCE_ROOT,train_and_evaluate
def run_task(parameters,output_dir:Path,checkpoint_dir:Path):
    if set(parameters)!={"expected_manifest_sha256","expected_code_sha"}:raise ValueError("latent-memory bridge parameters mismatch")
    return train_and_evaluate(Path("/jump-runs")/SOURCE_ROOT,output_dir,parameters["expected_manifest_sha256"],parameters["expected_code_sha"])
def main():
